...
prob3 step n = if n == 1 then 0 else 1 + prob3 step (step n)
...
getDivs  = getDivisors 2
  where
    getDivisors :: Integer -> Integer -> [Integer]
    getDivisors _ 1 = []
    getDivisors divisor n 
      |divisor * divisor > n = [n]
      |mod n divisor == 0 = divisor : getDivisors divisor (div n divisor)
      |otherwise = getDivisors (divisor + 1) n